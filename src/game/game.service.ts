    ...
    this.players.forEach((Player:Player)=>{
      ...
      this.players.forEach(player => {
        if (player.role === Role.Mafia) {
            server.to(player.clientId).emit('roleAssignment', { playerName: player.name, role: 'Mafia' });
            ...
            switch (player.role) {
                case Role.Citizen:
                  server.to(player.clientId).emit('roleAssignment', { playerName: player.name, role: 'Citizen' });
                    break;
                case Role.Doctor:
                  server.to(player.clientId).emit('roleAssignment', { playerName: player.name, role: 'Doctor' });
                    break;
                case Role.Police:
